...
using CruisePMS.MultiTenancy.Demo;
...
namespace CruisePMS.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
        ...
        public async Task<Permission[]> GetGrantedPermssionByRole(Role role, int? newTenantId)
        {
            using (_unitOfWorkManager.Current.SetTenantId(newTenantId))
            {
                return (await _roleManager.GetGrantedPermissionsAsync(role)).Where(x => x.MultiTenancySides != Abp.MultiTenancy.MultiTenancySides.Host).ToArray();
            }

        }

        public IQueryable<Role> GetCurrentUserRoles()
        {
            return _roleManager.Roles;
        }

        public async Task SetGranterPermissionByRoleAsync(Role role, IEnumerable<Permission> grantedPermissionNames)
        {
            await _roleManager.SetGrantedPermissionsAsync(role, grantedPermissionNames);
        }
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            ...
            var error = LocalizationManager.GetSource(CruisePMSConsts.LocalizationSourceName).GetString("FreeEditionsCannotHaveTrialVersions");
